...
float SingleValue::getEstimateValue(float size, const RuleSet& ruleSet, Shape* shape) {
...
float ValueSet::getEstimateValue(float size, const RuleSet& ruleSet, Shape* shape) {
	float sum = 0.0f;
	for (int i = 0; i < values.size(); ++i) {
		if (values[i]->type == Value::TYPE_ABSOLUTE) {
			sum += ruleSet.evalFloat(values[i]->value, shape);
		} else if (values[i]->type == Value::TYPE_RELATIVE) {
			sum += ruleSet.evalFloat(values[i]->value, shape) * size;
	...
	for (int i = 0; i < sizes.size(); ++i) {
		if (sizes[i]->type == Value::TYPE_ABSOLUTE) {
			...
			if (sizes[i]->repeat) {
				...
				float s = sizes[i]->getEstimateValue(size - regular_sum, ruleSet, shape);
				int num = (size - regular_sum - floating_sum * floating_scale) / s;
				for (int k = 0; k < num; ++k) {
					decodeSplitSizes((size - regular_sum - floating_sum * floating_scale) / num, sizes[i]->values, temp_names, ruleSet, shape, decoded_sizes, decoded_output_names);
				}
			} else {
				std::vector<Value*> temp_ratios(1);
				...
				decodeSplitSizes(size - regular_sum - floating_sum * floating_scale, temp_ratios, temp_names, ruleSet, shape, decoded_sizes, decoded_output_names);
			}
		}
	}
}
