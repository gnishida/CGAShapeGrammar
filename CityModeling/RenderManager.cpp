...
GeometryObject::GeometryObject(const std::vector<procedural_generation::Vertex>& vertices, bool lighting) {
...
void GeometryObject::addVertices(const std::vector<procedural_generation::Vertex>& vertices) {
...
void GeometryObject::createVAO() {
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(procedural_generation::Vertex) * vertices.size(), vertices.data(), GL_STATIC_DRAW);

	// configure the attributes in the vao
	glEnableVertexAttribArray(0);
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(procedural_generation::Vertex), 0);
	glEnableVertexAttribArray(1);
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(procedural_generation::Vertex), (void*)offsetof(procedural_generation::Vertex, normal));
	glEnableVertexAttribArray(2);
	glVertexAttribPointer(2, 4, GL_FLOAT, GL_FALSE, sizeof(procedural_generation::Vertex), (void*)offsetof(procedural_generation::Vertex, color));
	glEnableVertexAttribArray(3);
	glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, sizeof(procedural_generation::Vertex), (void*)offsetof(procedural_generation::Vertex, texCoord));
...
void RenderManager::init(const std::string& vertex_file, const std::string& geometry_file, const std::string& fragment_file, bool useShadow, int shadowMapSize) {
	...
	// INIT SECOND PASS
	// Quad
	std::vector<procedural_generation::Vertex> vert(4);
	vert[0] = procedural_generation::Vertex(glm::vec3(-1, -1, 0), glm::vec3(0, 0, 0), glm::vec4(0, 0, 0, 1), glm::vec2(0, 0));
	vert[1] = procedural_generation::Vertex(glm::vec3(1, -1, 0), glm::vec3(0, 0, 0), glm::vec4(0, 0, 0, 1), glm::vec2(1, 0));
	vert[2] = procedural_generation::Vertex(glm::vec3(1, 1, 0), glm::vec3(0, 0, 0), glm::vec4(0, 0, 0, 1), glm::vec2(1, 1));
	vert[3] = procedural_generation::Vertex(glm::vec3(-1, 1, 0), glm::vec3(0, 0, 0), glm::vec4(0, 0, 0, 1), glm::vec2(0, 1));
	...
	glBufferData(GL_ARRAY_BUFFER, sizeof(procedural_generation::Vertex)*vert.size(), vert.data(), GL_STATIC_DRAW);
...
void RenderManager::addFaces(const std::string& name, const procedural_generation::Faces& faces, bool lighting) {
	for (const auto& face : faces) {
		...
		for (int i = 0; i < face.vertices.size() - 2; i++) {
			...
			vertices[1] = face.vertices[i + 1];
			vertices[2] = face.vertices[i + 2];
			addObject(name.c_str(), face.texturePath.c_str(), vertices, lighting);
		}
	}
}

void RenderManager::addObject(const QString& object_name, const QString& texture_file, const std::vector<procedural_generation::Vertex>& vertices, bool lighting) {
	GLuint texId;
...
void RenderManager::centerObjects() {
	...
	for (auto it = objects.begin(); it != objects.end(); ++it) {
		for (auto it2 = it.value().begin(); it2 != it.value().end(); ++it2) {
			for (int k = 0; k < it2->vertices.size(); ++k) {
				minPt.x = (std::min)(minPt.x, it2->vertices[k].pos.x);
				minPt.y = (std::min)(minPt.y, it2->vertices[k].pos.y);
				minPt.z = (std::min)(minPt.z, it2->vertices[k].pos.z);
				maxPt.x = (std::max)(maxPt.x, it2->vertices[k].pos.x);
				maxPt.y = (std::max)(maxPt.y, it2->vertices[k].pos.y);
				maxPt.z = (std::max)(maxPt.z, it2->vertices[k].pos.z);
			}
		}
	}
	...
	for (auto it = objects.begin(); it != objects.end(); ++it) {
		for (auto it2 = it.value().begin(); it2 != it.value().end(); ++it2) {
			for (int k = 0; k < it2->vertices.size(); ++k) {
				it2->vertices[k].pos = (it2->vertices[k].pos - center) * scale;
			}
		}
	}
}
