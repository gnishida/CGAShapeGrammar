...
public:
	GLuint vao;
	GLuint vbo;
	std::vector<procedural_generation::Vertex> vertices;
	bool lighting;
	bool vaoCreated;
	bool vaoOutdated;
...
public:
	GeometryObject();
	GeometryObject(const std::vector<procedural_generation::Vertex>& vertices, bool lighting = true);
	void addVertices(const std::vector<procedural_generation::Vertex>& vertices);
	void createVAO();
};

class RenderManager {
public:
	enum { RENDERING_MODE_BASIC = 0, RENDERING_MODE_SSAO, RENDERING_MODE_CONTOUR, RENDERING_MODE_LINE, RENDERING_MODE_HATCHING, RENDERING_MODE_SKETCHY };
...
public:
	...
	void addFaces(const std::string& name, const procedural_generation::Faces& faces, bool lighting);
	void addObject(const QString& object_name, const QString& texture_file, const std::vector<procedural_generation::Vertex>& vertices, bool lighting);
	void removeObjects();
